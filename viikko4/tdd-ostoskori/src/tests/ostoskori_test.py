...
class TestOstoskori(unittest.TestCase):
    ...
    def test_kaksi_lisättyä_tuotetta_kaksi_tuotetta_korissa(self):
        maito = Tuote("Maito", 3)
        self.kori.lisaa_tuote(maito)
        self.kori.lisaa_tuote(maito)
        self.assertEqual(self.kori.tavaroita_korissa(), 2)

    def test_kahden_eri_tuotteen_lisääminen_osatoskorin_hinta_sama_kuin_tuotteiden_summa(self):
        maito = Tuote("Maito", 3)
        olut = Tuote("Olut", 2)
        self.kori.lisaa_tuote(maito)
    ...
    def test_tuotteen_lisaamisen_jalkeen_ostoskori_sisaltaa_yhden_ostoksen(self):
        maito = Tuote("Maito", 3)
        self.kori.lisaa_tuote(maito)
        self.assertEqual(len(self.kori.ostokset()), 1)

    def test_tuotteen_lisaamisen_jalkeen_kori_sisaltaa_ostoksen_lukumaaraltaan_yksi(self):
        maito = Tuote("Maito", 3)
        self.kori.lisaa_tuote(maito)
        nimi = self.kori.ostokset()[0].tuote.nimi()
        lkm = self.kori.ostokset()[0].lukumaara()
